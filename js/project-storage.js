import {S} from "./state.js?v=0.8.21.8-20260829-1425";
import {snapshotProject,restoreProject,clearHistory} from "./history.js?v=0.8.21.8-20260829-1425";
import {clearAllGeometry,validateGeometryState} from "./geometry.js?v=0.8.21.8-20260829-1425";
import {clearWalls} from "./walls.js?v=0.8.21.8-20260829-1425";
import {resetDrawingCore} from "./drawing-core.js?v=0.8.21.8-20260829-1425";
...
export function duplicateStoredProject(id,name=null){
  ...
  const copy=envelope(cleanName(name)||`${src.project.name} kopie`,null,null,JSON.parse(JSON.stringify(src.data)),{geo:src.project.geo||null,relocalization:src.project.relocalization||null});
...
export function newProject(name="Nieuw project"){
  ...
  try{
    ...
    const ts=nowIso();Object.assign(S.project,{schemaVersion:1,id:newId(),name:cleanName(name)||"Nieuw project",createdAt:ts,updatedAt:ts,lastSavedAt:null,dirty:true,recoveryAvailable:false,loadedFrom:"new",geo:null,relocalization:{references:[],active:false,captured:[],lastResult:null,mode:"auto"}});
...
export async function importProjectFile(file){
  ...
  const imported=envelope(idExists?`${src.project.name} import`:src.project.name,idExists?newId():src.project.id,src.project.createdAt,JSON.parse(JSON.stringify(src.data)),{geo:src.project.geo||null,relocalization:src.project.relocalization||null});
