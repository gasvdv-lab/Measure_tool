...
const pages=["home","measure","stake","newline","constraint","objects","line","point","shapecreate","clear"];
let menuStack=["home"];
...
export function initUI(){
  ...
  $("useReferenceBtn").onclick=()=>{setReferenceLine(S.selectedLineId);closeMenu();};$("deleteLineBtn").onclick=()=>{deleteLineRaw(S.selectedLineId);showPage("objects");};
  ...
  $("undoBtn").onclick=()=>{const a=S.undo.pop();if(a?.type==="createdLine"){deleteLineRaw(a.lineId);deletePointRaw(a.endId);}closeMenu();};
  $("clearAllBtn").onclick=()=>showPage("clear");$("cancelClearBtn").onclick=()=>showPage("home",false);$("confirmClearBtn").onclick=()=>{clearAllGeometry();S.draw.active=false;resetCurrent();closeMenu();$("stage").textContent="Alles gewist";$("hint").textContent="AR blijft actief. Plaats opnieuw punt A.";};
