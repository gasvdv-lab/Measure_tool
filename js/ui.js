import {S,$,fmt,getPoint,getLine,getContour} from "./state.js?v=0.8.8.1-20260822-1615";
import {setConstraint,setReferenceLine,updateReferenceStatus} from "./constraints.js?v=0.8.8.1-20260822-1615";
import {startMeasureNew,startMeasureFrom,startStake,startContinuous,undoContinuous,finishContinuous,placePoint,resetCurrent} from "./drawing.js?v=0.8.8.1-20260822-1615";
import {createShape,deleteLineRaw,deletePointRaw,clearAllGeometry,dispose} from "./geometry.js?v=0.8.8.1-20260822-1615";
import {startAR,leaveAR,applyZoom} from "./ar.js?v=0.8.8.1-20260822-1615";
import {updatePlacementUI,placeParametricNext} from "./placement.js?v=0.8.8.1-20260822-1615";
import {createWall,getWall,deleteWall,toggleWall,wallsUsingLine,clearWalls} from "./walls.js?v=0.8.8.1-20260822-1615";
...
function bind(id,event,handler){
  const node=el(id);
  if(!node){
    console.warn(`[UI 0.8.8.1] ontbrekend element: #${id}`);
    return false;
  }
...
function openPlacementPage(){
  ...
  if(el("placementContextInfo")) el("placementContextInfo").textContent=p?`Vertrekpunt: ${p.name}`:"Plaats of selecteer eerst een vast vertrekpunt.";
...
export function initUI(){
  ...
  bind("measureNewBtn","click",()=>{S.placementMode="manual";startMeasureNew();closeMenu();});
  ...
  bind("newIndependentBtn","click",()=>{S.placementMode="manual";startMeasureNew();closeMenu();});
  ...
  bind("stakeNewBtn","click",()=>{S.placementMode="manual";const m=readStake();if(m){startStake(null,m);closeMenu();}});
  ...
  bind("continuousBtn","click",()=>{S.placementMode="manual";startContinuous();closeMenu();});
  ...
  bind("placementApplyBtn","click",()=>{
    try{placeParametricNext();closeMenu();}
