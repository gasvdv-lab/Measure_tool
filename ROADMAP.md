...
Measure AR is een domeinonafhankelijk **Spatial Design & Reality Platform**. De app verbindt digitale objecten en geometrie met de echte fysieke ruimte zodat de gebruiker kan **meten, plaatsen, maken, ervaren en controleren** — van “past dit object hier?” tot een volledig ruimtelijk ontwerp.

### Vaste hoofdpijlers
1. **MEASURE** — werkelijkheid meten: afstand, hoogte, hoek, oppervlak, volume en vrije ruimte.
2. **PLACE** — digitale objecten/CAD 1:1 previewen en in de echte ruimte plaatsen.
3. **CREATE** — tekenen, parametrisch ontwerpen, Spatial Composer en AI Builder.
4. **EXPERIENCE** — toekomstige situaties 1:1 beleven, varianten/scènes vergelijken en presenteren.
5. **VERIFY** — werkelijkheid met ontwerp/CAD vergelijken en afwijkingen controleren.

**CAPTURE** is een contextuele ondersteunende laag voor het vastleggen van ruimtes, vlakken, oppervlakken en later reconstruction/scanning. **AI** is een ondersteunende intentielaag; exacte geometrie, constraints, berekeningen en projectstate blijven deterministisch eigendom van de core.

### Architectuurprincipes
- **Project Space blijft geometrische waarheid.** AR/World Lock/Relocalization bepalen alleen de pose in de actuele fysieke wereld.
- Nieuwe functies groeien richting één universeel **Spatial Object**-model voor geometrie, CAD, AI-objecten, bibliotheekobjecten en metingen.
- Universele **Select/Edit**-interactie moet objecten kunnen selecteren, verplaatsen, roteren, kopiëren, meten, verbergen en bewerken.
- **Spatial Relationships / Constraint Solver** beheert relaties zoals tegen, boven, tussen, parallel, loodrecht, gecentreerd en vaste afstand.
- **Environment Context** beschrijft relevante vloer-, wand-, plafond-, terrein- en vrije-ruimtecontext.
- **Clearance/Collision** beantwoordt niet alleen “past het?”, maar ook hoeveel vrije ruimte overblijft en of bewegende objecten kunnen functioneren.
- **Variants/Scenes** maken alternatieven vergelijkbaar zonder duplicatie van volledige projecten.
- **Project Timeline** ondersteunt bestaand → ontwerp → uitgevoerd/as-built.
- **Presentation/Experience Mode** verbergt technische bediening wanneer het project alleen beleefd of getoond wordt.
- **Object Library** ondersteunt eigen objecten, parametrische basisobjecten, geïmporteerde objecten en later templates.
- Delen/exporteren blijft een projectfunctie, geen aparte domeinworkflow.
- Wegenis-, calculatie-, aanbestedings- en bedrijfsspecifieke wegenbouwworkflows horen **niet** in Measure AR.

## Ontwikkelstrategie naar v1.0

### Fase A — v0.8.x: navigatie, CAD en spatial fundament
- **[x] v0.8.29.2 — Startinterface / Navigation Core**: taakgerichte startinterface rond Meten, Plaatsen, Maken, Ervaren en Controleren. Plaatsen gaat rechtstreeks naar CAD Preview; Ervaren/Controleren blijven zichtbaar als toekomstige richtingen. Bestaande AR/World Lock/geometry-core niet herschreven.
- **[ ] v0.8.29.3 — CAD Preview 2.0**: origin/assen, grondvlak, wireframe/transparantie en voorbereiden van een bewust gekozen plaatsingspunt.
- **v0.8.29.3 — CAD Preview 2.0**: origin/assen, grondvlak, wireframe/transparantie en plaatsingspunt controleren/kiezen.
- **v0.8.30 — CAD Inspect**: CAD-objectselectie, hiërarchie/modelinfo, hide/show/isolate/transparantie.
- **v0.8.31 — Universal Select/Edit**: uniforme selectie en transform-acties voor ondersteunde Spatial Objects.
- **v0.8.32 — Work Planes & Environment Context**: horizontale, verticale en vrije werkvlakken plus basiscontext van de omgeving.
- **Spatial Restore blocker**: `Project op locatie plaatsen` / reference capture definitief repareren vóór functies die betrouwbare herplaatsing vereisen.

### Fase B — v0.9.x: geïntegreerde Measure / Place / Create
- **Measure Engine 2.0**: reality↔reality, CAD↔CAD en reality↔CAD; afstand, hoogte, hoek, helling, oppervlak en volume.
- **Spatial Object Core**: nieuwe modules conform één gedeeld objectcontract; bestaande data gecontroleerd migreren, niet in één risicovolle rewrite.
- **Spatial Relationships / Constraint Solver**.
- **Clearance & Collision Engine**: passen, botsingen, resterende vrije ruimte en bewegingszones.
- **AI Builder 2.0 / Spatial Composer**: parametrische objecten en natuurlijke opdrachten bovenop deterministische geometry/constraints.
- **Object Library**: basisobjecten, eigen objecten, templates en opgeslagen AI-objecten.
- **Project Explorer**: hiërarchie en gekoppelde selectie.
- **2D Plan View**: technische planweergave/bewerking van hetzelfde Project Space.
- **Variants / Scenes**: alternatieven binnen hetzelfde project.
- **Experience Mode**: 1:1 rondlopen/bekijken met vereenvoudigde presentatie-UI.
- **Verify / Reality ↔ Design**: ontwerp versus werkelijkheid, afwijkingen en toleranties.

### Fase C — v1.0: productbaseline
Geen feature-race. v1.0 is bereikt wanneer de volledige kernworkflow betrouwbaar is:

**Project → Capture/Measure → Place/Create → Experience → Verify → Save → sluiten → later correct herstellen → verderwerken.**

Voor v1.0 moeten minimaal stabiel zijn: projectopslag/recovery, Spatial Restore, World Lock, Measure Engine, CAD import/preview/placement/inspect, universele selectie/editing, basis Experience, Project Explorer en consistente mobiele UX. Daarna volgen v1.x/v2.0-uitbreidingen zoals geavanceerde reconstruction, Surface Engine, richer materials, tijd/scenario's, geavanceerde generative spatial design, collaboration en andere XR-platformen.

## Release- en testdiscipline (blijft ongewijzigd)
- Elke release als downloadbare volledige ZIP.
- Cumulatief bijwerken: `README.md`, `ROADMAP.md`, `TESTING.md`, `MODULES.md`.
- Geen losse `TEST_RESULTS_*.md`-bestanden in release-ZIP's.
- Kleine/lokale wijziging: relevante module + directe dependencies gericht controleren.
- Mijlpaal/architectuurwijziging: bredere regressie.
- Altijd minimale eindcheck: gewijzigde JS-syntax, imports/referenties en ZIP-integriteit.
- Fysieke AR-tests blijven expliciete user-side gates met `AR-IN/A4`, `AR-IN/ROOM` en `AR-OUT`.

---

## Historiek / bestaande roadmap
